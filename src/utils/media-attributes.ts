...
function firstNonEmpty(state: MediaSourceState, keys: string[]): string | undefined {
...
export function getBestTitle(state: MediaSourceState): string | undefined {
  return firstNonEmpty(state, TITLE_ATTRS);
}

/** Untertitel: z.B. Künstler, Album, Staffel/Episode, Kanal */
export function getBestSubtitle(state: MediaSourceState): string | undefined {
  const attrs = state.attributes;
...
export function getBestImage(state: MediaSourceState): string | undefined {
...
export function getRelevantAttributes(state: MediaSourceState): Record<string, unknown> {
