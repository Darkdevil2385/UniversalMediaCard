...
export class UniversalMediaCard extends LitElement {
  ...
  private _renderAppIcon(): TemplateResult | typeof nothing {
    if (!this._config.show_app_icon) return nothing;
  ...
  private _renderNowPlaying(): TemplateResult | typeof nothing {
    const state = this._getActiveSourceState();
    const config = this._getActiveSourceConfig();
    if (!state || !config) return nothing;
    ...
    if (!title && !subtitle) return nothing;
